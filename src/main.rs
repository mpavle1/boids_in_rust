...
fn rotate_point(point: Vec2d, angle_rad: f64, d: f64) -> Vec2d {
    let cos_theta = angle_rad.cos();
    let sin_theta = angle_rad.sin();

    let new_x = point[0] - d * cos_theta;
    let new_y = point[1] - d * sin_theta;
...
impl Boid {
    fn new() -> Self {
        ...
        Self {
            ...
            velocity: 1.0,
            direction: PI * random.gen_range(0.0..=two_pi),
            color: [
    ...
    fn draw(&self, ctx: &Context, g: &mut G2d) {
        ...
        let a = rotate_point([x, y], FRAC_PI_2 + angle_rad, 5.0);
        let b = rotate_point([x, y], FRAC_PI_2 + angle_rad, -5.0);
        let c = rotate_point([x, y], angle_rad, 15.0);
    ...
    fn update(&mut self) -> () {
        ...
        let [new_x, new_y] = rotate_point([self.x, self.y], self.direction, self.velocity);
